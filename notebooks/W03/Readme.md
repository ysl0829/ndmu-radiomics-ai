...
本資料夾是第 3 週的課堂實作材料，主題是建立可攜、可重建、可重現的 AI 研究環境。
...
| 項目 | 位置 | 用途 |
|---|---|---|
| 套件清單 | `requirements.txt` | 建立 W03–W18 共用的完整環境 |
| 環境建置 notebook | `notebooks/W03/00_w03_setup.ipynb` | 驗證 GPU、Drive、路徑、種子與專案結構 |
| 除錯練習 notebook | `notebooks/W03/01_debug_challenge.ipynb` | 找出版本、路徑與隨機性問題 |
| 專案使用說明 | `docs/README.md` | 說明環境安裝、資料位置與程式執行順序 |
| 環境紀錄 | `results/YYYYMMDD_env.json` | 保存 Python、套件、GPU、種子與執行時間 |
| 標準資料夾 | `data/`、`notebooks/`、`src/`、`results/`、`docs/` | 分離原始資料、程式與分析結果 |

### 範例 1：`requirements.txt`

課程 GitHub 已提供 W03–W18 共用清單。學生不需要逐行手動安裝套件，使用：
...
版本應以 repository 內實際的 `requirements.txt` 為準，不要從投影片抄寫。清單包含
PyRadiomics，因此首次安裝可能需要編譯；它刻意不含 torch／torchvision，不要自行升級
Colab 預裝的 PyTorch。

### 範例 2：設定種子與建立環境紀錄

```python
from src.paths import get_root, stamped
from src.repro import RANDOM_STATE, env_report, set_seed

set_seed(42)
ROOT = get_root(project="radiomics_course")
env_report(save_to=ROOT / "results" / stamped("env", ext=".json"))
```

scikit-learn 的函式層級亂數仍須另外設定：
...
W03 只要求三行，不需要先寫成正式研究文件：

```markdown
# CT 術後復發預測

**環境怎麼裝**：使用 Colab；執行 `pip install -r requirements.txt`；不升級預裝的 PyTorch。

**資料放哪**：去識別化資料放在 `data/raw/`，病人使用 `SUB001`–`SUB120`；資料不提交至 Git。

**程式什麼順序跑**：`00_w03_setup.ipynb` → `01_preprocess_ct.ipynb` → `02_model.ipynb`。
```
...
├── data/
│   ├── raw/
│   │   └── SUB001/                    # 去識別化原始影像；唯讀、不提交至 Git
│   └── processed/
│       └── SUB001/                    # 可由程式重新產生的中間資料
├── notebooks/
│   ├── W03/
│   │   ├── 00_w03_setup.ipynb
...
- `data/raw/` 保持唯讀；任何處理結果放入 `data/processed/`。
- 病人只使用 `SUB001` 形式代號，不使用姓名或病歷號。
- `data/` 與身分對照表不得提交至公開 repository。
- 結果檔名包含日期、內容與重要參數，不覆寫舊結果。
- 所有路徑由 `ROOT` 組合，不寫死 `/content/...` 或個人電腦的絕對路徑。
